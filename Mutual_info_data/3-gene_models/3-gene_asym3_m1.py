...
def stat(var, param):
	...
	dstat[0] = 1./(1.+(var[1]/0.38254855)**n) /(1.+(var[2]/0.35595578)**n)
	dstat[1] = 1./(1.+(var[2]/0.43337526)**n) /(1.+(var[0]/0.41022626)**n)
	dstat[2] = 1./(1.+(var[0]/0.38015885)**n) /(1.+(var[1]/0.40378526)**n)
...
init_conc = np.genfromtxt('3-gene_asym3_m1_init_conc.txt', delimiter=',')
...
file = open("MI_data_model1_s"+str(steepness_index+1)+".txt", "w")
